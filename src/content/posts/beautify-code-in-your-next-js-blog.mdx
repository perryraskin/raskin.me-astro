...
date: March 4, 2020
tags: [nextjs, tailwindcss, highlightjs]
description: After some trial and error, I styled custom code blocks using Highlight.js and  TailwindCSS
---
...
```jsx
<article className="markdown mb-10">
  <header>
    <h1 className="text-5xl">{frontmatter.title}</h1>
  </header>
  <div className="text-neutral-400 my-auto mb-5 text-sm font-semibold">{reformatDate(frontmatter.date)}</div>
  <div>
